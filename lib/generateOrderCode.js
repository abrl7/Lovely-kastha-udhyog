...
export async function generateOrderCode() {
  ...
  const prefix = `LKU-${currentYear}-`;

  // Find the highest-numbered code this year rather than counting documents.
  // Counting breaks when orders are deleted (gaps in the sequence cause
  // re-use of numbers that already exist). Max-based generation always
  // moves forward regardless of deletions.
  ...
  let nextNumber = 1;
  if (lastOrder) {
    const lastNumber = parseInt(lastOrder.orderCode.split("-")[2], 10);
